...
from makrell.makrellpy.stdmetas import _mr_defs
...
class CompilerContext:
    def __init__(self, compile_mr):
        ...
        self.suffix_transforms = {
            "strsuffix": dict(_mr_defs.get("strsuffix", {})),
            "intsuffix": dict(_mr_defs.get("intsuffix", {})),
            "floatsuffix": dict(_mr_defs.get("floatsuffix", {})),
        }
...
"""
    ...
    def quote(
        ...
        raw: bool = False,
        quote_depth: int = 0,
        allow_special_forms: bool = True,
    ) -> Node:
        # TODO: cleanup raw usage
        def eval_in_meta(expr_node: Node):
            prev = self.cc.running_in_meta
            self.cc.running_in_meta = True
            try:
                pyast = self.cc.compile_mr(expr_node, self.cc)
            finally:
                self.cc.running_in_meta = prev
            if isinstance(pyast, list):
                raise Exception("Expected expression in unquote")
            expr = py.Expression(pyast)
            py.fix_missing_locations(expr)
            code = compile(expr, "", mode="eval")
            return eval(code, self.globals, self.symbols)

        match n:
            case CurlyBrackets(nodes) if (allow_special_forms and (
                get_identifier(nodes[0], "unquote")
                or get_identifier(nodes[0], "$")
            )):
                if quote_depth > 0:
                    # Unquote only applies at the current quote level.
                    # Nested quotes keep inner unquote forms as syntax.
                    return self.quote(
                        n,
                        raw=raw,
                ...
                try:
                    ...
                    # Keep original behavior for symbolic splicing cases
                    # where runtime meta locals are intentionally unresolved.
                    return unquoted[0]
            case CurlyBrackets(nodes) if (allow_special_forms and
                                          get_identifier(nodes[0], "quote")):
                quoted = self.cc.operator_parse(regular(nodes[1:]))
                if len(quoted) == 0:
                    return self.quote(
                        None,
                        raw=raw,
