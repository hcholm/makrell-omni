...
def compile_mr(n: Node, cc: CompilerContext) -> py.AST | list[py.AST] | None:
    ...
    def resolve_meta_head(head: Node) -> Any:
        if isinstance(head, Identifier):
            return cc.meta.symbols.get(head.value)
        if isinstance(head, BinOp) and head.op == ".":
            owner = resolve_meta_head(head.left)
            if owner is None:
                return None
            return getattr(owner, head.right.value, None)
        return None

    def compile_suffix_transform(kind: str, suffix: str, value: str) -> py.expr | None:
        transform = cc.suffix_transforms[kind].get(suffix)
        if transform is None:
            return None
        transformed = transform(value)
        if isinstance(transformed, py.expr):
            return transformed
        if isinstance(transformed, Node):
            return c(transformed)
        return py.Constant(transformed)
    ...
    match n:
        case Identifier(value):
            ...
            if not cc.running_in_meta and value in cc.meta.symbols:
                # meta identifier
                mv = cc.meta.symbols[value]
                if isinstance(mv, Node):
                    return c(mv)
                if isinstance(mv, (bool, int, float, str, complex, type(None))):
                    return pb.constant(mv)
            # regular identifier
        ...
        case Number():
            # numeric constant
            if n.suffix:
                kind = "intsuffix" if "." not in n.value and "e" not in n.value and "E" not in n.value else "floatsuffix"
        ...
        case n if isinstance(n, (str, int, float, bool, complex)) or n is None:
            return pb.constant(n)
