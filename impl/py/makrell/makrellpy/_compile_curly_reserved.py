...
def compile_curly_reserved(n: CurlyBrackets, cc: CompilerContext, compile_mr, opp_nodes) -> py.AST | list[py.AST] | None:
    ...
    def eval_meta_expr(expr_node: Node):
        expr_ast = c(expr_node)
        if isinstance(expr_ast, list):
            raise Exception("Expected expression, got statements")
        expr = py.Expression(expr_ast)
        py.fix_missing_locations(expr)
        code = compile(expr, "", "eval")
        return eval(code, cc.meta.globals, cc.meta.symbols)
    ...
    match n0.value:
        ...
        case "dataclass":
            if len(nodes) < 2:
                raise Exception(f"Invalid number of arguments to dataclass: {parlen}")
            name = get_identifier(nodes[1], require=True).value
            body = []
            for field_node in nodes[2:]:
                fbo = get_binop(field_node, ":", require=True)
                field_name = get_identifier(fbo.left, require=True).value
                field_type = c(fbo.right)
                ann = py.AnnAssign(
                    target=py.Name(field_name, py.Store()),
            ...
            if not body:
                body = [py.Pass()]
            cls = py.ClassDef(name=name, bases=[], keywords=[], body=body, decorator_list=[py.Name("dataclass", py.Load())])
            imp = py.ImportFrom(module="dataclasses", names=[py.alias("dataclass")], level=0)
            return [imp, cls]
        ...
        case "quote":
            q_nodes = regular(flatten(nodes[1:]))
            if len(q_nodes) == 0:
                return c(cc.meta.quote(None))
            if len(q_nodes) == 1:
                return c(cc.meta.quote(q_nodes[0]))
            return py.List([c(cc.meta.quote(qn)) for qn in q_nodes], py.Load())
        ...
        case "def":
            ...
            if len(reg_nodes) >= 3:
                ...
                match deftype:
                    ...
                    case "pattern":
                        if len(nodes) != 5:
                            raise Exception(f"Invalid number of arguments to pattern: {len(nodes)}")
                        pattern_name = nodes[2].value
                        can_handle = eval_meta_expr(nodes[3])
                        make_test = eval_meta_expr(nodes[4])
                        if not callable(can_handle):
                            raise Exception("Pattern can_handle must be callable")
                        if not callable(make_test):
                            raise Exception("Pattern make_test must be callable")
                        pattype = [pattern_name, can_handle, make_test]
                        pattern_types = cc.meta.globals.get("_match_pattern_types")
                        if pattern_types is None:
                            pattern_types = cc.meta.symbols.get("_match_pattern_types")
                        if not isinstance(pattern_types, list):
                            raise Exception("_match_pattern_types is not available in meta context")
                        pattern_types.insert(0, pattype)
                        return pb.pass_()
                    
                    case "strsuffix":
                        if len(nodes) != 4:
                            raise Exception(f"Invalid number of arguments to strsuffix: {len(nodes)}")
                        suffix = nodes[2].value
                        transform = eval_meta_expr(nodes[3])
                        if not callable(transform):
                            raise Exception("String suffix transform must be callable")
                        cc.suffix_transforms["strsuffix"][suffix] = transform
                        return pb.pass_()
                    
                    case "intsuffix":
                        if len(nodes) != 4:
                            raise Exception(f"Invalid number of arguments to intsuffix: {len(nodes)}")
                        suffix = nodes[2].value
                        transform = eval_meta_expr(nodes[3])
                        if not callable(transform):
                            raise Exception("Int suffix transform must be callable")
                        cc.suffix_transforms["intsuffix"][suffix] = transform
                        return pb.pass_()
                    
                    case "floatsuffix":
                        if len(nodes) != 4:
                            raise Exception(f"Invalid number of arguments to floatsuffix: {len(nodes)}")
                        suffix = nodes[2].value
                        transform = eval_meta_expr(nodes[3])
                        if not callable(transform):
                            raise Exception("Float suffix transform must be callable")
                        cc.suffix_transforms["floatsuffix"][suffix] = transform
                        return pb.pass_()
